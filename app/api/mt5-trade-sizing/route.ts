...
8) **Multi-symbol, no close:** Opening risk on one symbol does **not** require closing open trades on **another** symbol; sizing should assume **add-on** exposure. If combined margin would be unsafe (very low free margin or margin level near a margin-call zone), **reduce** this symbol’s `lotSize` and/or `numberOfTrades` so the user can add without the system having to free margin by mass-closing other positions.
